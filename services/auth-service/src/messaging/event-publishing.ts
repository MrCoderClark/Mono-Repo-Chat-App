...
export const initPublisher = async () => {
    ...
    logger.info('Auth service RabbitMQ publisher initialized successfully');
};

export const publishUserRegisteredEvent = (payload: AuthUserRegisteredPayload) => {
    if (!channel) {
        logger.warn('RabbitMQ channel is not initialized. Cannot publush message.');
        return;
    }
    ...
    if (!published) {
        logger.error(`Failed to publish ${AUTH_USER_REGISTERED_ROUTING_KEY} event`);
    }
};
